...
let test_web_admin_submission_create_proxy_is_available () =
  let app = Toolkit.Web_app.make (web_config ()) in
  let submission =
    request_handler app ~meth:`POST ~target:"/_admin_api/submissions"
...
let tests : test_case list =
  [
    ...
    ( "web_admin_submission_create_proxy_is_available",
      test_web_admin_submission_create_proxy_is_available );

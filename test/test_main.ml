...
let base_config ?(auth_rate_limit_max_requests = 10) ?(rate_limit_max_requests = 100)
    () : Toolkit.Config.t =
  {
    ...
    admin_email = Some "admin@recognita.xyz";
    admin_password = Some "adminpass123";
  }
...
let make_fixture ?(auth_rate_limit_max_requests = 10)
  ...
  let bootstrap_result =
    Lwt_main.run
      (Toolkit.Auth_service.bootstrap_admin { repo; clock; config; mailer })
  in
  (match bootstrap_result with
  | Ok () -> ()
  | Error app_error ->
      fail
        ("Failed to bootstrap admin in tests: "
        ^ Toolkit.App_error.message app_error));
  let app =
...
let test_admin_lists_and_moderates_users () =
  ...
  let admin_login = login fixture "admin" "adminpass123" in
